...
function countOccurences (array) {
    ...
    var current = null;
    var cnt = 0;
    //loop through letters in current string (sorted)
    for (var i = 0; i < array.length; i++) {
        if (array[i] != current) {
            if (cnt === 2) {
                occurrences[current] = cnt
            }
            if (cnt === 3) {
                occurrences[current] = cnt
            }
    ...
    // now get the distinct values in the object and add to the doubles and triples variables accordingly
    _.unique(Object.values(occurrences)).forEach(el => {
        if (el === 2) {
            theseDoubles +=1
        } else if (el === 3) {
            theseTriples +=1
    ...
    totalDoubles += theseDoubles
    totalTriples += theseTriples
    occurrences = {}
    theseDoubles = 0;
    theseTriples = 0;
...
for (let i = 0; i < input.length; i++) {
    ...
    countOccurences(letters); // counts up letter occurrences in the current string

}

var checksum = totalDoubles*totalTriples
// console.log(`There were ${totalDoubles} doubles and ${totalTriples} triples.`);
console.log(`Your checksum is ${checksum}`);
